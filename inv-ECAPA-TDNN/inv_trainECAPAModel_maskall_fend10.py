...
from inv_ECAPAModel import ECAPAModel
...
parser.add_argument("--init_factor",     type=float, default=100.)
parser.add_argument("--end_factor",     type=float, default=10.)
...
if args.initial_state != "":
	print("Model state %s loaded from previous state!"%args.initial_state)
	epoch = s.load_states(args.initial_state) + 1

EERs = []
score_file = open(args.score_save_path, "a+")

mask_epoch = args.max_epoch
mask_ratio_begin = 0.5
factor_begin = args.init_factor
factor_end = args.end_factor

while(1):
    factor = max(factor_begin - (factor_begin / mask_epoch * epoch), factor_end)
    #mask_ratio = max(mask_ratio_begin - (mask_ratio_begin / mask_epoch) * epoch, 0.)
    mask_ratio = mask_ratio_begin

	## Training for one epoch
    loss, lr, acc = s.train_network(epoch = epoch, loader = trainLoader,
        factor = factor, mask_ratio = mask_ratio)

    ## Evaluation every [test_step] epochs
    if epoch % args.test_step == 0:
        s.save_parameters(args.model_save_path + "/model_%04d.model"%epoch)
        s.save_states(epoch, args.model_save_path + "/model_%04d.optim"%epoch)
