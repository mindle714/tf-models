...
class ECAPAModel(nn.Module):
	...
	def save_states(self, epoch, path):
		torch.save({
			'epoch': epoch,
	...
	def load_states(self, path):
		pre_states = torch.load(path)
		epoch = pre_states['epoch']
		self.optim.load_state_dict(pre_states['optim'])
		self.scheduler.load_state_dict(pre_states['scheduler'])
		return epoch
