...
import wavlm
if args.timit:
  m = wavlm.wavlm_phone(num_class=50, use_last=False, use_layers=3)
  if args.ssl_fix_adapt:
    m_masked = wavlm.wavlm_phone(num_class=50, use_last=False, use_layers=3)
  is_ctc = False
elif args.speech_command:
  m = wavlm.wavlm_phone(num_class=10, use_last=False, use_layers=3, single_output=True)
  if args.ssl_fix_adapt:
    m_masked = wavlm.wavlm_phone(num_class=10, use_last=False, use_layers=3, single_output=True)
  is_ctc = False
elif args.voxceleb:
  m = wavlm.wavlm_phone(num_class=1251, use_last=False, use_layers=3, single_output=True)
  if args.ssl_fix_adapt:
    m_masked = wavlm.wavlm_phone(num_class=1251, use_last=False, use_layers=3, single_output=True)
  is_ctc = False
else:
  m = wavlm.wavlm_phone(use_last=False, use_layers=12)
  if args.ssl_fix_adapt:
    m_masked = wavlm.wavlm_phone(use_last=False, use_layers=12)
  is_ctc = True
...
if args.warm_start is not None:
  ...
  if args.ssl_rand is not None:
    ...
    for i, conv in enumerate(m.wavlm.wavlm.fe.conv_layers):
      add_rand(conv.conv)
      # TODO lnorm
      
    # TODO lnorm
    add_rand(m.wavlm.wavlm.fp.proj)
    add_rand(m.wavlm.wavlm.enc.emb.conv)
    # TODO lnorm

    for i, layer in enumerate(m.wavlm.wavlm.enc.layers):
