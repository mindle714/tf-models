...
if args.val_tfrec is not None:
  ...
  val_dataset = parse_data.gen_val(val_tfrec_list,
    (val_samp_len if no_spec else val_spec_len),
    no_spec=no_spec, batch_size=args.batch_size, seed=seed)
...
if args.ssl_fix_adapt:
  if args.noise_list is not None:
    ...
    for data in _adapt_dataset:
      ...
      _spec = _spec_dict['spec']
      _spec_len = _spec_dict['spec_len']
      adapt_dataset.append({
        'spec': _spec, 'spec_len': _spec_len,
...
if args.timit:
  ...
  if args.ssl_fix_adapt:
    m_masked = tera.tera_phone(num_class=50, use_last=True)
  is_ctc = False
elif args.speech_command:
  m = tera.tera_phone(num_class=10, use_last=True, single_output=True)
  if args.ssl_fix_adapt:
    m_masked = tera.tera_phone(num_class=10, use_last=True, single_output=True)
  is_ctc = False
elif args.voxceleb:
  m = tera.tera_phone(num_class=1251, use_last=True, single_output=True)
  if args.ssl_fix_adapt:
    m_masked = tera.tera_phone(num_class=1251, use_last=True, single_output=True)
  is_ctc = False
else:
  m = tera.tera_phone(use_last=False)
  if args.ssl_fix_adapt:
    m_masked = tera.tera_phone(use_last=False)
  is_ctc = True
...
@tf.function
def run_adapt_step(step, spec, txt,
                   spec_len, txt_len,
                   stop_grad=False):
    ...
    if args.ssl_fix_adapt and ssl_fix_adapt:
      ...
      for adapt_data in adapt_dataset:
        loss_1, loss_2 = run_adapt_step(
          ...
          adapt_data["spec"], adapt_data["txt"],
          adapt_data["spec_len"], adapt_data["txt_len"])
