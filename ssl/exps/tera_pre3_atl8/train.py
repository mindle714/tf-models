...
parser.add_argument("--begin-weight-pow", type=float, required=False, default=1.)
parser.add_argument("--weight-pow-step", type=int, required=False, default=50000)
...
loss_ema = tf.Variable(1., dtype=tf.float32) 
loss_frozen_ema = tf.Variable(1., dtype=tf.float32) 

@tf.function
def run_step(step, pcm, ref, weight_pow, training=True):
  ...
  with tf.GradientTape() as tape, log_writer.as_default():
    ...
    weight = tf.math.exp(-tf.math.abs(
      (tf.math.log(loss) - tf.math.log(loss_ema)) - (tf.math.log(loss_frozen) - tf.math.log(loss_frozen_ema))))
    weight = tf.stop_gradient(weight)
    weight = tf.math.pow(weight, weight_pow)

    loss_frozen_ema.assign_sub(
      (loss_frozen_ema - tf.math.reduce_mean(loss_frozen)) * (1. - args.lr_beta), use_locking=True)
    loss_ema.assign_sub(
      (loss_ema - tf.math.reduce_mean(loss)) * (1. - args.lr_beta), use_locking=True)
...
for idx, data in enumerate(dataset):
  ...
  weight_pow = args.begin_weight_pow
  if args.weight_pow_step > 0:
    weight_pow = args.begin_weight_pow - (idx - init_epoch) * (args.begin_weight_pow / args.weight_pow_step)
    weight_pow = max(weight_pow, 0.)

  loss, closs, weight = run_step(tf.cast(idx, tf.int64), data["pcm"], data["ref"], tf.cast(weight_pow, tf.float32))
  log_writer.flush()

  if idx > init_epoch and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] closs[{:.2f}] lr[{:.2e}] weight[{}] pow[{}]".format(
      idx, loss, closs, lr.numpy(), ",".join([str("{:.2f}".format(e.numpy())) for e in weight]), weight_pow))
