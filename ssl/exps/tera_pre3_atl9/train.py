...
def run_step(step, pcm, ref, training=True):
  ...
  loss_frozen_unb = exp(loge(loss_frozen) - loge(loss_frozen_ema))

  with tf.GradientTape() as tape, log_writer.as_default():
    loss, closs = m((pcm, ref), training=training)
    loss_unb = exp(loge(loss) - loge(loss_ema))

    def approx_crossprod(e1, e2):
      return exp(-tf.math.abs(loge(e1)-loge(e2)))

    loss_frozen_unb = tf.math.reduce_mean(loss_frozen_unb)
    zw = approx_crossprod(loss_frozen_unb, tf.math.reduce_mean(loss_unb))
    xw = approx_crossprod(loss_unb, tf.math.reduce_mean(loss_unb))
    xz = approx_crossprod(loss_unb, loss_frozen_unb) 

    weight = zw / (xw + xz + 1e-9)
