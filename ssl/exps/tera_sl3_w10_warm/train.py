...
parser.add_argument("--begin-ssl", type=int, required=False, default=10000)
...
for idx, data in enumerate(dataset):
  ...
  loss, closs, sloss = run_step(tf.cast(idx, tf.int64), data["pcm"], data["ref"])
  log_writer.flush()

  if idx > init_epoch and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] closs[{:.2f}] sloss[{:.2f}] lr[{:.2e}]".format(
      idx, loss, closs, sloss, lr.numpy()))
