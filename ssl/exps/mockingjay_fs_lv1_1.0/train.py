...
import mockingjay
m = mockingjay.mockingjay_unet(args.beta_r, args.beta_m)
...
def run_step(step, pcm, ref, training=True):
  for i in range(3):
    m.mockingjay.enc.factors[i].assign(
...
for idx, data in enumerate(dataset):
  ...
  if idx > init_epoch and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] closs[{:.2f}] lr[{:.2e}] flr[{}]".format(
      idx, loss, closs, lr.numpy(), ",".join([str("{:.2f}".format(e.numpy())) for e in m.mockingjay.enc.factors])))
