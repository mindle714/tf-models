...
class mockingjay(tf.keras.layers.Layer):
  def __init__(self, n_fft, hop_len, 
               ...
               beta_m=[0.9, 0.9, 0.9],
               *args, **kwargs):
    super(mockingjay, self).__init__(*args, **kwargs)
...
class mockingjay_unet(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.mockingjay_frozen = mockingjay(self.n_fft, self.hop_len, name='mockingjay_frozen')
    self.mockingjay = mockingjay(self.n_fft, self.hop_len, self.beta_r, self.beta_m)
  ...
  def call(self, inputs, training=None):
    ...
    fs = None
    if ref is not None:
      fs = self.mockingjay_frozen(ref, training=False)
      fs = [tf.stop_gradient(f) for f in fs]

    xs = self.mockingjay((x, fs))
    x = xs[-1]
