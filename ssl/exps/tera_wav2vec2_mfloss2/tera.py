...
class tera_unet(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.tera = tera(self.n_fft, self.hop_len, name='tera_freeze')
    self.wav2vec2 = wav2vec2(name='wav2vec2_freeze')
    self.tera_gen = tera(self.n_fft, self.hop_len)
  ...
  def call(self, inputs, training=None):
    ...
    if ref is not None:
      ...
      def feat_loss(ref, x, model):
        '''
        fs = model(ref, training=False)
        xs = model(x, training=False)

        feat_loss = [tf.math.reduce_mean(
          tf.math.abs(tf.stop_gradient(_f) - _x)) for _f, _x in zip(fs, xs)]
        feat_loss = sum(feat_loss)
        return feat_loss
        '''
        with tf.GradientTape() as tape:
          f, fs = model(ref, training=False)
        f_grads = tape.gradient(f, fs)
        
        with tf.GradientTape() as tape:
          x, xs = model(x, training=False)
        x_grads = tape.gradient(x, xs)

        l2norm = tf.math.l2_normalize
        fx_sim = [-tf.math.reduce_sum(l2norm(_f, -1) * l2norm(_x, -1), -1) for _f, _x in zip(fs, xs)]

        feat_loss = [tf.math.reduce_mean(_fx_sim) for _fx_sim in fx_sim]
        feat_loss = sum(feat_loss) / len(feat_loss)
        return feat_loss

      tera_feat_loss = feat_loss(ref, x, self.tera)
      wav2vec2_feat_loss = feat_loss(tf_expd(ref, -1), tf_expd(x, -1), self.wav2vec2)

      return samp_loss + spec_loss, cirm_loss, tera_feat_loss + wav2vec2_feat_loss
