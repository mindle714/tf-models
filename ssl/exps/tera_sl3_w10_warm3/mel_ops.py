"""mel conversion ops from tensorflow; fixed for htk standard"""
...
def _mel_to_hertz(mel_values, name=None):
  ...
  with ops.name_scope(name, 'mel_to_hertz', [mel_values]):
    ...
    return _MEL_BREAK_FREQUENCY_HERTZ * (
        math_ops.pow(10.0, mel_values / _MEL_HIGH_FREQUENCY_Q) - 1.0
    )

def log10(x):
  numerator = math_ops.log(x)
  denominator = math_ops.log(math_ops.cast(10, dtype=numerator.dtype))
...
def _hertz_to_mel(frequencies_hertz, name=None):
  ...
  with ops.name_scope(name, 'hertz_to_mel', [frequencies_hertz]):
    ...
    return _MEL_HIGH_FREQUENCY_Q * log10(
...
def linear_to_mel_weight_matrix(num_mel_bins=20,
  ...
  with ops.name_scope(name, 'linear_to_mel_weight_matrix') as name:
    ...
    spectrogram_bins = array_ops.expand_dims(linear_frequencies, 1)

    # Compute num_mel_bins triples of (lower_edge, center, upper_edge). The
    # center of each band is the lower and upper edge of the adjacent bands.
    # Accordingly, we divide [lower_edge_hertz, upper_edge_hertz] into
    # num_mel_bins + 2 pieces.
    band_edges = shape_ops.frame(
        math_ops.linspace(_hertz_to_mel(lower_edge_hertz),
                          _hertz_to_mel(upper_edge_hertz),
                          num_mel_bins + 2), frame_length=3, frame_step=1)
    band_edges = _mel_to_hertz(band_edges)

    # Split the triples up and reshape them into [1, num_mel_bins] tensors.
    lower_edge, center, upper_edge = tuple(array_ops.reshape(
        t, [1, num_mel_bins]) for t in array_ops.split(
            band_edges, 3, axis=1))

    # Calculate lower and upper slopes for every spectrogram bin.
    # Line segments are linear in the mel domain, not Hertz.
    lower_slopes = (spectrogram_bins - lower_edge) / (
        center - lower_edge)
    upper_slopes = (upper_edge - spectrogram_bins) / (
        upper_edge - center)
