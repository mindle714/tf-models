...
for idx, data in enumerate(dataset):
  ...
  loss, floss = run_step(tf.cast(idx, tf.int64), data["pcm"], data["ref"])
  log_writer.flush()

  if idx > init_epoch and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] floss[{:.2f}] lr[{:.2e}]".format(
      idx, loss, floss, lr.numpy()))
