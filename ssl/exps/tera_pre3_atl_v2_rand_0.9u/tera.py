...
class tera_unet(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.prj1 = conv1d(256, 5, strides=3, **conv_opt, name='prj1_frozen')
    self.prj2 = conv1d(128, 5, strides=3, **conv_opt, name='prj2_frozen')
    self.prj3 = conv1d(64, 5, strides=3, **conv_opt, name='prj3_frozen')
    self.prj4 = tf.keras.layers.Dense(1, use_bias=True, name='prj4_frozen')

  def clip_prj(self):
    for p in [self.prj1, self.prj2, self.prj3, self.prj4]:
      w, b = p.get_weights()
      w = tf.nn.relu(w)
      p.set_weights([w, b])
  ...
  def sub_prj_noise(self, noises):
    for noise, p in zip(noises, 
      [self.prj1, self.prj2, self.prj3, self.prj4]):
      w, b = p.get_weights()
      p.set_weights([w-noise, b])
  ...
  def call(self, inputs, get_ptarget=False, training=None):
    ...
    def get_prj_loss(x):
      ...
      prj_x = self.prj4(prj_x)
      prj_loss = tf.math.reduce_mean(prj_x, -1)

      return prj_loss

    if get_ptarget:
      x = self.tera_frozen(x, training=False)[-1]
      # feat is frozen when extracting prj target
      x = tf.stop_gradient(x)
      prj_loss = get_prj_loss(x)
      return prj_loss
