...
class gnorm(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    if isinstance(input_shape, tuple): self.dim = input_shape[0][-1]
    else: self.dim = input_shape[-1]
  ...
  def call(self, inputs, training=None):
    if isinstance(inputs, tuple):
      ...
      if self.num_groups == 1:
        ...
        x = tf.reshape(x, tf.concat([
          tf.shape(x)[:2], [self.num_groups, group_dim]], 0))
        m, v = tf.nn.weighted_moments(x, [1, 3], mask, keepdims=True)
        gamma = tf.reshape(self.gamma, [1, 1, self.num_groups, group_dim])
        beta = tf.reshape(self.beta, [1, 1, self.num_groups, group_dim])

        norm = gamma * (x-m) / tf.math.sqrt(v + self.eps) + beta
        return tf.reshape(norm, tf.concat([tf.shape(x)[:2], [self.dim]], 0))
    ...
    gamma = tf.reshape(self.gamma, [1, 1, self.dim])
    beta = tf.reshape(self.beta, [1, 1, self.dim])
