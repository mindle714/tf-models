...
loss_ema = tf.Variable(np.ones(args.batch_size), dtype=tf.float32) 
loss_frozen_ema = tf.Variable(np.ones(args.batch_size), dtype=tf.float32) 
...
def run_step(step, pcm, ref, training=True):
  ...
  with tf.GradientTape() as tape, log_writer.as_default():
    ...
    loss_frozen_ema.assign_sub(
      (loss_frozen_ema - loss_frozen) * (1. - args.lr_beta), use_locking=True)
    loss_ema.assign_sub(
      (loss_ema - loss) * (1. - args.lr_beta), use_locking=True)
