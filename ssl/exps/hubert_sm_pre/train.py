...
import hubert
m = hubert.hubert_unet(beta_r=args.beta_r, beta_m=args.beta_m)
...
def run_step(step, pcm, ref, training=True):
  ...
  if training:
    ...
    #assert len(m.trainable_weights) - len(train_weights) == 65
...
if args.warm_start is not None:
  ...
  _ = m((_in, None))
  _ = m.hubert((_in, None))
  #_ = m.hubert_frozen((_in, None))
  ckpt.read(args.warm_start).expect_partial()#.assert_consumed()
...
for idx, data in enumerate(dataset):
  ...
  if idx > init_epoch and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] lr[{:.2e}] flr[{}]".format(
      idx, loss, lr.numpy(),
      ",".join([str("{:.2f}".format(e.numpy())) for e in m.hubert.fe.factors])))
