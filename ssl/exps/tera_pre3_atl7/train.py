...
parser.add_argument("--batch-factor", type=int, required=False, default=4)
...
parser.add_argument("--weight-pow", type=float, required=False, default=1.)
...
dataset = parse_data.gen_train(tfrec_list, samp_len,
  batch_size=args.batch_factor*args.batch_size, seed=seed)
...
@tf.function
def get_data(step, pcm, ref):
  loss_frozen, _ = m_frozen((pcm, ref), training=False)
  loss, _ = m((pcm, ref), training=False)

  weight = tf.math.exp(-tf.math.abs(
    (tf.math.log(loss) - tf.math.log(loss_ema)) - (tf.math.log(loss_frozen) - tf.math.log(loss_frozen_ema))))
  weight = tf.math.pow(weight, args.weight_pow)

  log_inv_weight = -tf.math.log(weight)
  idxs = tf.random.categorical([log_inv_weight], args.batch_size)[0]

  pcm_idxs = tf.gather(pcm, idxs)
  ref_idxs = tf.gather(ref, idxs)
  return pcm_idxs, ref_idxs
...
for idx, data in enumerate(dataset):
  ...
  pcm, ref = get_data(tf.cast(idx, tf.int64), data["pcm"], data["ref"])
  loss, closs, weight = run_step(tf.cast(idx, tf.int64), pcm, ref)
  log_writer.flush()
