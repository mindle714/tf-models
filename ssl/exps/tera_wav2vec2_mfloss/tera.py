...
class tera_unet(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training=None):
    ...
    if ref is not None:
      ...
      fs = self.tera(ref, training=False)
      xs = self.tera(x, training=False)

      tera_feat_loss = [tf.math.reduce_mean(
        tf.math.abs(tf.stop_gradient(_f) - _x)) for _f, _x in zip(fs, xs)]
      tera_feat_loss = sum(tera_feat_loss)
      
      _, fs = self.wav2vec2(tf_expd(ref, -1), training=False)
      _, xs = self.wav2vec2(tf_expd(x, -1), training=False)

      wav2vec2_feat_loss = [tf.math.reduce_mean(
        tf.math.abs(tf.stop_gradient(_f) - _x)) for _f, _x in zip(fs, xs)]
      wav2vec2_feat_loss = sum(wav2vec2_feat_loss)
