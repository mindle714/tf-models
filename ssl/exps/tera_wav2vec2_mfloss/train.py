...
def run_step(step, pcm, ref, training=True):
  with tf.GradientTape() as tape, log_writer.as_default():
    loss, closs, floss = m((pcm, ref), training=training)
    loss = tf.math.reduce_mean(loss)
    tf.summary.scalar("loss", loss, step=step)
    closs = tf.math.reduce_mean(closs)
    tf.summary.scalar("closs", closs, step=step)
    floss = tf.math.reduce_mean(floss)
    tf.summary.scalar("floss", floss, step=step)
    total_loss = loss + closs + floss

  if training:
    train_weights = [e for e in m.trainable_weights if 'freeze' not in e.name]
    assert len(m.trainable_weights) - len(train_weights) == 61
    grads = tape.gradient(total_loss, train_weights)
    grads, _ = tf.clip_by_global_norm(grads, 5.)
    opt.apply_gradients(zip(grads, train_weights))

  return loss, closs, floss
...
if args.warm_start is not None:
  ...
  _ = m(_in)
  _ = m.tera(_in)
  _ = m.wav2vec2(_in.reshape([args.batch_size, -1, 1]))
...
for idx, data in enumerate(dataset):
  ...
  loss, closs, floss = run_step(tf.cast(idx, tf.int64), data["pcm"], data["ref"])
  log_writer.flush()

  if idx > init_epoch and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] closs[{:.2f}] floss[{:.2f}] lr[{:.2e}]".format(
      idx, loss, closs, floss, lr.numpy()))
