...
if len(args.beta_r) == 0:
  args.beta_r = [0.999, 0.999, 0.999]
assert len(args.beta_r) == 3

if len(args.beta_m) == 0:
  args.beta_m = [0.99, 0.99, 0.99]
assert len(args.beta_m) == 3
...
@tf.function
def run_step(step, pcm, ref, step_odd=0, training=True):
  for i in range(3):
  ...
  if step_odd == 0:
    m.tera.enc.factors[1].assign(0.)
  else:
    m.tera.enc.factors[0].assign(0.)
...
for idx, data in enumerate(dataset):
  ...
  loss, closs = run_step(tf.cast(idx, tf.int64), data["pcm"], data["ref"], step_odd=idx%2)
  log_writer.flush()

  if idx > init_epoch and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] closs[{:.2f}] lr[{:.2e}] flr[{}]".format(
      idx, loss, closs, lr.numpy(), ",".join([str("{:.2f}".format(e.numpy())) for e in m.tera.enc.factors])))
