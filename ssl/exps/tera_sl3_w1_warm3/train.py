...
parser.add_argument("--ssl-weight", type=float, required=False, default=10)
...
def run_step(step, pcm, ref, training=True):
  with tf.GradientTape() as tape, log_writer.as_default():
    ...
    ssl_weight = 0.
    if step > args.begin_ssl and step < args.end_ssl:
      if args.ssl_decay:
        slope = args.ssl_weight / (args.end_ssl - args.begin_ssl)
        ssl_weight = -slope * tf.cast(step, tf.float32) + (slope * args.end_ssl)
      else:
        ssl_weight = args.ssl_weight
    total_loss = loss + closs + ssl_weight * sloss
  ...
  return loss, closs, sloss, ssl_weight
...
for idx, data in enumerate(dataset):
  ...
  loss, closs, sloss, sw = run_step(tf.cast(idx, tf.int64), data["pcm"], data["ref"])
  log_writer.flush()

  if idx > init_epoch and idx % args.eval_step == 0:
    logger.info("gstep[{}] loss[{:.2f}] closs[{:.2f}] sloss[{:.2f}] lr[{:.2e}] sr[{:.2e}]".format(
      idx, loss, closs, sloss, lr.numpy(), sw))
