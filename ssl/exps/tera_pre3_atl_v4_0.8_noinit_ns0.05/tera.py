...
class tera_unet(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    conv_opt = dict(padding='same', use_bias=True)

    self.lrelu = tf.keras.layers.LeakyReLU()
    self.prj1 = conv1d(256, 4, strides=2, **conv_opt, name='prj1_frozen')
    self.prj2 = conv1d(128, 4, strides=2, **conv_opt, name='prj2_frozen')
    self.prj3 = conv1d(64, 4, strides=2, **conv_opt, name='prj3_frozen')
    self.prj4 = conv1d(32, 4, strides=2, **conv_opt, name='prj4_frozen')
    self.prj5 = tf.keras.layers.Dense(1, use_bias=True, name='prj5_frozen')

  def clip_prj(self):
    for p in [self.prj1, self.prj2, self.prj3, self.prj4, self.prj5]:
      w, b = p.get_weights()
      w = tf.nn.relu(w)
      p.set_weights([w, b])
  ...
  def init_prj(self):
    for p in [self.prj1, self.prj2, self.prj3, self.prj4, self.prj5]:
      ...
      if isinstance(p, conv1d):
        minmax = 1. / tf.cast(tf.shape(w)[0] * tf.shape(w)[1], tf.float32)
        minmax = tf.math.sqrt(minmax)
        w_new = tf.random.uniform(tf.shape(w),
          minval=-minmax, maxval=minmax)

      else:
        minmax = tf.math.sqrt(6. / tf.cast(tf.shape(w)[0] + tf.shape(w)[1], tf.float32))
        w_new = tf.random.uniform(tf.shape(w),
          minval=-minmax, maxval=minmax)

      p.set_weights([w_new, b])

  def add_prj_noise(self, rate=0.05):
    noises = []
    ...
    for p in [self.prj1, self.prj2, self.prj3, self.prj4, self.prj5]:
      ...
      if isinstance(p, conv1d):
        minmax = 1. / tf.cast(tf.shape(w)[0] * tf.shape(w)[1], tf.float32)
        minmax = tf.math.sqrt(minmax)
        noise = tf.random.uniform(tf.shape(w),
          minval=-minmax, maxval=minmax)
        noise *= rate
      ...
      noises.append(noise)
      p.set_weights([w+noise, b])
    
    return noises

  def sub_prj_noise(self, noises):
    for noise, p in zip(noises, 
      [self.prj1, self.prj2, self.prj3, self.prj4, self.prj5]):
      w, b = p.get_weights()
      p.set_weights([w-noise, b])
  
  def call(self, inputs, get_ptarget=False, training=None):
    ...
    def get_prj_loss(xs):
      x = tf.concat(xs, -1)
      prj_x = self.lrelu(self.prj1(x))
      prj_x = self.lrelu(self.prj2(prj_x))
      prj_x = self.lrelu(self.prj3(prj_x))
      prj_x = self.lrelu(self.prj4(prj_x))
      prj_x = tf.reshape(prj_x, [tf.shape(prj_x)[0], -1])

      prj_x = self.prj5(prj_x)
      prj_loss = tf.math.reduce_mean(prj_x, -1)

      return prj_loss

    if get_ptarget:
      xs = self.tera_frozen(x, training=False)
      # feat is frozen when extracting prj target
      x = tf.stop_gradient(x)
      prj_loss = get_prj_loss(xs)
      return prj_loss
    ...
    if ref is not None:
      ...
      return samp_loss + spec_loss, cirm_loss, prj_loss
