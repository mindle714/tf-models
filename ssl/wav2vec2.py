...
class wav2vec2_unet(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.wav2vec2_frozen = wav2vec2(self.num_enc_layer, name='wav2vec2_frozen')
    self.wav2vec2 = wav2vec2(self.num_enc_layer, self.beta_r, self.beta_m)
  ...
  def call(self, inputs, training=None):
    ...
    _fes = None
    if ref is not None:
      _fes, _ = self.wav2vec2_frozen(ref, training=False)
      _fes = [tf.stop_gradient(f) for f in _fes]

    fes, encs = self.wav2vec2((x, _fes))
    x = encs[-1]
