...
class lnormconv1d(tf.keras.layers.Layer):
  def __init__(self, ksize, stride=2, *args, **kwargs):
    self.ksize = ksize
    self.stride = stride
    super(lnormconv1d, self).__init__()
...
class posconvemb(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training=None):
    ...
    for conv in self.convs:
      ...
      x = self.gelu(self.norm(conv(x_pad)))
...
class data2vec_seq(tf.keras.layers.Layer):
  def __init__(self, *args, **kwargs):
    super(data2vec_seq, self).__init__()

  def build(self, input_shape):
    self.data2vec = data2vec()
...
class data2vec_unet(tf.keras.layers.Layer):
  def __init__(self, *args, **kwargs):
    super(data2vec_unet, self).__init__()
    self.layer = 7
