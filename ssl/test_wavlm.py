...
import torch
m = torch.load("/home/hejung/transformers/examples/pytorch/audio-classification/wavlm-base-ft-keyword-spotting/checkpoint-12772/pytorch_model.bin")
...
for i, conv in enumerate(model.wavlm.fe.conv_layers):
  prefix = 'wavlm.feature_extractor.conv_layers'
  load_conv('{}.{}.conv'.format(prefix, i), conv.conv)
  if i == 0:
    load_norm('{}.{}.layer_norm'.format(prefix, i), conv.norm)

prefix = 'wavlm.feature_projection'
load_norm('{}.layer_norm'.format(prefix), model.wavlm.fp.norm)
load_affine('{}.projection'.format(prefix), model.wavlm.fp.proj)

prefix = 'wavlm.encoder'
...
model.wavlm.enc.emb.conv.set_weights([w, b])
load_norm('{}.layer_norm'.format(prefix), model.wavlm.enc.norm)

for i, layer in enumerate(model.wavlm.enc.layers):
  prefix = 'wavlm.encoder.layers.{}'.format(i)
